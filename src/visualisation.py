...
def plot_avg_lap_times(
    ...
    if race_filter or 'race_name' in df_filtered.columns:
        ...
        plt.title(f'Average Lap Times - {race_name} ({season})', 
...
def plot_temp_vs_lap_scatter(
    ...
    plt.ylabel('Average Lap Time (seconds)', fontsize=12, fontweight='bold')
    plt.title(f'Track Temperature vs Lap Time\nCorrelation: {correlation:.4f}', 
              fontsize=14, fontweight='bold', pad=20)
...
def plot_lap_progression(
    ...
    if df.empty:
        print(f"No lap data available for {season} round {round_num} with specified drivers")
        return
    ...
    plt.title(f'Lap Time Progression - Season {season} Round {round_num}', 
              fontsize=14, fontweight='bold', pad=20)
...
def plot_tyre_performance(
    df: pd.DataFrame,
    output_filename: str = "tyre_performance.png"
) -> None:
    """
    ...
    plt.ylabel('Average Lap Time (seconds)', fontsize=12, fontweight='bold')
    plt.title('Tyre Compound Performance Comparison', 
              fontsize=14, fontweight='bold', pad=20)
...
def plot_grid_vs_finish(
    df: pd.DataFrame,
    top_n: int = 20,
    output_filename: str = "grid_vs_finish.png"
) -> None:
    """
    ...
    plt.title('Top Position Changes: Grid to Finish', 
              fontsize=14, fontweight='bold', pad=20)
    
    # Set y-axis labels
    labels = [f"{row.driver_code} - {row.race_name[:15]}" 
...
def generate_all_visualizations(conn: sqlite3.Connection, calculation_results: dict) -> None:
    """
    ...
    if not calculation_results['avg_lap_times'].empty:
        ...
        if not df.empty:
            ...
            top_drivers = race_data.nsmallest(2, 'avg_lap_time_ms')['driver_code'].tolist()
...
if __name__ == "__main__":
    ...
    from . import db_utils, calculations
