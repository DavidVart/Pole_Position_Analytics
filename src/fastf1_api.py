...
def extract_lap_data(session) -> List[Dict]:
    ...
    for idx, lap in laps_df.iterrows():
        try:
            ...
            fresh_tyre = 1 if lap.get('FreshTyre') == True else 0
...
def store_fastf1_data(conn: sqlite3.Connection) -> int:
    ...
    for year, event_name, race_full_name in events_to_process:
        ...
        if round_row:
            ...
            if metadata:
                print(f"  Successfully demonstrated requests.get for {metadata['race_name']}")
        ...
        for session_type in session_types:
            ...
            session_id = get_or_create_session(conn, race_id, session_type, weather_data)
