...
from . import jolpica_api
from . import fastf1_api
...
def main():
    ...
    print(f"    Status: {'✓ MET' if req['results_requirement_met'] else '✗ NOT MET (need more runs)'}")
    print()
    print(f"  LapTimes rows:   {req['laps_count']:6d} / 100 required")
    print(f"    Status: {'✓ MET' if req['laps_requirement_met'] else '✗ NOT MET (need more runs)'}")
    print()
    ...
    if req['results_requirement_met'] and req['laps_requirement_met']:
        ...
        print(f"  • Database:      data/f1_project.db")
        print(f"  • CSV files:     outputs/csv/")
